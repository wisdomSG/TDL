...
public class AwsS3Service {
    ...
    // Aws에 프로필 파일 업로드
    public String uploadProfileFile(MultipartFile multipartFile){
        String imageName;
    ...
    public List<String> uploadFile(List<MultipartFile> multipartFiles){
        ...
        multipartFiles.forEach(file -> {
            String fileNames = createFileName(file.getOriginalFilename());
            ObjectMetadata objectMetadata = new ObjectMetadata();
            objectMetadata.setContentLength(file.getSize());
            objectMetadata.setContentType(file.getContentType());

            try(InputStream inputStream = file.getInputStream()){
                amazonS3.putObject(new PutObjectRequest(bucket, fileNames, inputStream, objectMetadata)
                        .withCannedAcl(CannedAccessControlList.PublicRead)); // 업로드한 파일을 모두가 읽을 수 있게 설정
            } catch (IOException e){
                throw new ResponseStatusException(HttpStatus.INTERNAL_SERVER_ERROR, "파일 업로드에 실패했습니다.");
            }
