...
public class AdminSelectUserResponseDto { // 유저관리에서 유저 누르면 유저 정보 유저게시글 전부 반환하거나 , 검색반환
    ...
    public AdminSelectUserResponseDto(AdminUserResponseDto selectUser, List<AdminPostResponseDto> selectUserPosts) {  // 유저관리에서 유저 눌렀을때
        this.adminUser = selectUser;
        this.adminPostList = selectUserPosts;
    }
    public AdminSelectUserResponseDto(List<AdminPostResponseDto> selectUserPosts , PageInfo info , List<CategoryContentsResponseDto> categoryDtoList) { // 검색했을때
        this.adminPostList = selectUserPosts;
        this.info = info;
